...
class WebStreamer:
    frame_bytes = b''
    # Condition lets streaming threads block until a new frame arrives instead of polling
...
<body>
    ...
    <div>
        ...
        <a class="btn btn-end"   href="/end">End</a>
                ...
                try:
                    while True:
                        # Block until a new frame is available (timeout avoids permanent hang on idle)
    ...
    try:
        ...
        try:
            out = subprocess.check_output(
                ['/usr/bin/python3', '-c', 'import sys; print(repr(sys.path))']
            ).decode('utf-8').strip()
        ...
        print(f"Picamera2 import failed (falling back to OpenCV): {e}", file=sys.stderr)
    ...
    try:
        while True:
            ...
            if system_state == SystemState.IDLE:
                ...
                # Publish a heartbeat once per second so the dashboard stays live
                now = time.time()
            ...
            # ── CALIBRATING ───────────────────────────────────────────────────
                ...
                if elapsed < config.calibration_seconds:
                    ...
                    # Publish calibration progress event (seconds_outside_zone carries remaining time)
                    event_bus.publish(SystemEvent(
                        timestamp=time.time(),
                        system_state=SystemState.CALIBRATING,
