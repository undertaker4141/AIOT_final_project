#!/usr/bin/env bash
# =============================================================
#  AIOT Node A（Pi 版）— 樹莓派一鍵安裝與開機自啟動設定腳本
...
# setup_rpi.sh 放在 Node_A_Pi/ 目錄內
NODE_A_PI_DIR="$SCRIPT_DIR"
...
echo -e "\n${BOLD}=== AIOT Node A（Pi 版）樹莓派安裝腳本 ===${RESET}"
echo -e "  專案目錄：${NODE_A_PI_DIR}"
...
VENV_PYTHON="$NODE_A_PI_DIR/.venv/bin/python"
...
[Unit]
Description=AIOT Node A（Pi 版）— 學習專注與坐姿監測系統
...
Type=simple
User=${RUN_USER}
WorkingDirectory=${NODE_A_PI_DIR}
...
echo -e "  Node A（Pi 版）已在背景執行，開機後自動啟動"
echo -e ""
...
echo -e ""
echo -e "  ${YELLOW}提示：Node C 的 .env 中 VITE_NODE_A_HOST 需改為 ${PI_IP}${RESET}"
